...
I want a development deployment of RHTAP where I can:
...
the PaC integration features of RHTAP, you will need to use repositories from this organization
(or fork existing repositories into this org).

Copy the organization name to `preview.env`. For example:

```shell
export MY_GITHUB_ORG=<gh_username>-rhtap
```
...
```shell
export IMAGE_CONTROLLER_QUAY_ORG=<quay_username>-rhtap
```
...
You can create the application in your organization settings (<https://github.com/organizations/MY_RHTAP_ORG/settings/apps>)
...
* **GitHub Application Name**: use a unique name, e.g. `<gh_username>-rhtap PaC`
...
* Run `hack/build/build-via-appstudio.sh https://github.com/MY_RHTAP_ORG/devfile-sample-python-basic`
